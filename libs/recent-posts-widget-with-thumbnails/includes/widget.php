...
<div id="rpwwt-<?php echo $args[ 'widget_id' ];?>" class="rpwwt-widget rec-post">
	...
	<?php while ( $r->have_posts() ) : $r->the_post(); ?>
		<li<?php 
			...
			if ( ! $hide_title ) {
				?><h1 class="rpwwt-post-title"><?php if ( $post_title = $this->get_the_trimmed_post_title( $post_title_length ) ) { echo $post_title; } else { the_ID(); } ?></h1><?php
			}
			?></a><?php 
			...
			if ( $show_date ) : 
				?><small><?php echo get_the_date(); ?><?php 
			endif;
			if ( $show_author ) : 
				?><span> <?php echo $this->get_the_author(); ?></span></small><?php 
			endif;			
			if ( $show_excerpt ) : 
